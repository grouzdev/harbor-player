...
`benchmark` проверяет синтетическую базу из 100 000 треков / 10 000 альбомов, актуальный фильтр папки и виртуализацию в Chrome. Это **не измерение импорта 100 000 реальных файлов**. Обычный запуск пишет отчёт в `.test-data/benchmark/catalog-benchmark.json`; `npm run benchmark:baseline` — единственная команда, которая обновляет версионируемый `verification/catalog-benchmark.json`.
