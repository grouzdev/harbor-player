...
function header(data: Buffer, offset: number): Header | null {
  if (!data.subarray(offset, offset + 3).equals(Buffer.from("ID3")))
    return null;
...
function frames(data: Buffer, offset: number, tag: Header) {
  ...
  for (let cursor = 0; cursor < body.length;) {
    ...
    const size =
      tag.version === 3
        ? body.readUInt32BE(cursor + 4)
        : readSyncsafe(body.subarray(cursor + 4, cursor + 8));
...
function frontCover(frameBytes: Buffer, version: Version) {
  ...
  const size =
    version === 3
      ? frameBytes.readUInt32BE(4)
      : readSyncsafe(frameBytes.subarray(4, 8));
...
function writeId3v1(data: Buffer, patch: TagPatch) {
  if (
    data.length < 128 ||
    data.subarray(-128, -125).toString("ascii") !== "TAG"
  )
    return data;
  ...
  if (patch.year !== undefined)
    set(93, 4, patch.year ? String(patch.year) : "");
  if (patch.trackNumber !== undefined && tag[125] === 0)
    tag[126] = patch.trackNumber || 0;
  if (patch.genres !== undefined) {
    const index =
      patch.genres.length === 1 ? id3v1Genres.indexOf(patch.genres[0]) : -1;
...
export async function writeMp3TagsLosslessly(
  file: string,
  patch: TagPatch,
  counts: { track?: number; disc?: number },
) {
  ...
  for (const { offset, tag } of blocks) {
    ...
    let current = frames(data, offset, tag).filter((entry) => {
      ...
      return (
        !replace.has(id) &&
        !(patch.cover !== undefined && frontCover(entry, tag.version))
      );
    });
    for (const [key, value] of Object.entries(patch)) {
      const id = ids(tag.version)[key];
      if (!id || value === undefined || (Array.isArray(value) && !value.length))
        continue;
      const count =
        key === "trackNumber"
          ? counts.track
    ...
    output.push(
      Buffer.from([0x49, 0x44, 0x33, tag.version, 0, 0]),
      syncsafe(body.length + padding.length),
      body,
      padding,
    );
  }
