...
const frameIds: Record<string, string> = {
  title: "TIT2", artists: "TPE1", albumTitle: "TALB", albumArtists: "TPE2",
  genres: "TCON", year: "TDRC", trackNumber: "TRCK", discNumber: "TPOS",
};
const syncsafe = (value: number) =>
  Buffer.from([(value >> 21) & 127, (value >> 14) & 127, (value >> 7) & 127, value & 127]);
const readSyncsafe = (value: Buffer) => {
  if (value.some((byte) => byte & 128)) throw new Error("Повреждён размер ID3-тега");
...
function textFrame(id: string, values: string[], version: Version) {
  ...
  const body = version === 4
    ...
    : Buffer.concat([Buffer.from([1, 255, 254]), Buffer.from(text, "utf16le")]);
  const size = Buffer.alloc(4);
  if (version === 4) syncsafe(body.length).copy(size); else size.writeUInt32BE(body.length);
...
export async function writeId3InPlace(file: string, patch: TagPatch): Promise<{ fast: boolean; reason?: string }> {
  if (patch.cover !== undefined) return { fast: false, reason: "обложка обрабатывается отдельно" };
  const handle = await open(file, "r+");
  try {
    const header = Buffer.alloc(10);
    if ((await handle.read(header, 0, 10, 0)).bytesRead !== 10 || header.toString("ascii", 0, 3) !== "ID3")
    ...
    const replace = new Set(Object.keys(patch).map((key) => frameIds[key]).filter(Boolean));
    const frames: Buffer[] = [];
    for (let offset = 0; offset < body.length;) {
      if (body.subarray(offset, offset + 4).every((byte) => byte === 0)) break;
      if (offset + 10 > body.length) return { fast: false, reason: "неподдерживаемый ID3v2" };
      const id = body.subarray(offset, offset + 4).toString("ascii");
      const length = version === 4 ? readSyncsafe(body.subarray(offset + 4, offset + 8)) : body.readUInt32BE(offset + 4);
      const end = offset + 10 + length;
      if (!/^[A-Z0-9]{4}$/.test(id) || end > body.length) return { fast: false, reason: "неподдерживаемый ID3v2" };
    ...
    if (updated.length > size) return { fast: false, reason: "в ID3v2 недостаточно свободного места" };
    await handle.write(Buffer.concat([updated, Buffer.alloc(size - updated.length)]), 0, size, 10);
    await handle.sync();
    return { fast: true };
