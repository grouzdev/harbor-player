...
export interface Library { id: string; name: string; path: string; available: boolean; lastScan: string | null; trackCount: number }
export interface Track {
  id: string; libraryId: string; relativePath: string; title: string; artists: string[];
  albumTitle: string; albumArtists: string[]; albumKey: string; genres: string[];
  year: number | null; trackNumber: number | null; discNumber: number | null;
  duration: number; format: string; size: number; mtimeMs: number; coverId: string | null;
  available: boolean;
}
export interface Album { id: string; title: string; artists: string[]; year: number | null; coverId: string | null; trackCount: number }
...
export const tagPatchSchema = z.object({
  title: z.string().max(1000).optional(), artists: z.array(z.string().max(500)).max(100).optional(),
  albumTitle: z.string().max(1000).optional(), albumArtists: z.array(z.string().max(500)).max(100).optional(),
  genres: z.array(z.string().max(200)).max(100).optional(), year: z.number().int().min(0).max(9999).nullable().optional(),
  trackNumber: z.number().int().min(0).max(99999).nullable().optional(),
  discNumber: z.number().int().min(0).max(9999).nullable().optional(),
  cover: z.object({ data: z.string().max(14_000_000), mime: z.enum(['image/jpeg', 'image/png']) }).nullable().optional(),
}).strict();
...
export interface OperationItem {
  id: string; trackId: string | null; source: string; destination: string;
...
export interface OperationPreview {
  ...
  items: OperationItem[]; patch?: TagPatch; targetLibraryId?: string; restoreOf?: string;
}
export interface Job {
  id: string; kind: 'scan' | 'operation'; label: string; status: 'queued' | 'running' | 'done' | 'error';
