...
Первый самостоятельный этап P1 закрыт 18 сентября 2026 года: error semantics разделены, N+1 при выдаче треков устранён, а лимит очереди в 100 000 треков стал явным. Проверки этапа: `npm run typecheck`, `npm test` (111), `npm run build` и `npm run verify:tags` (7 форматов) — пройдены. Пункты P1.8–P3 остаются backlog следующего этапа.
...
4. Следующим этапом измерить facet-запросы, определить обслуживание SQLite и сделать CI явным quality gate.
