...
Пункты P1–P3 ниже остаются backlog следующего этапа; первым самостоятельным этапом рекомендуются error semantics, устранение N+1 и явный контракт лимита 100k.
...
### 5. Разделить ожидаемые ошибки API и внутренние сбои

**Подтверждено.** Глобальный error handler всегда отвечает `400`: условие в `src/server/app.ts:141-147` возвращает одинаковый код для Zod и всех остальных ошибок. В результате ошибка диска, SQLite, native module или программный дефект выглядит для клиента как неверный запрос. Текст внутренней ошибки полностью передаётся UI.

Предлагается ввести небольшой набор внутренних ошибок:

- validation — `400`;
- not found — `404`;
- conflict/current-state mismatch — `409`;
- unavailable/stopping — `503`;
- неизвестная ошибка — `500`, общий текст клиенту и полный stack в локальный журнал.

Fastify-ошибки с уже заданным `statusCode` нужно сохранять. Клиентскому `api()` достаточно продолжить читать поле `error`; публичный wire format менять не обязательно.

### 6. Убрать N+1 при загрузке треков

**Подтверждено.** `Catalog.tracks()` выполняет отдельный `SELECT DISTINCT format` для каждого уникального альбома текущей страницы (`src/server/database.ts:447-459`). Страница содержит до 200 треков, поэтому в худшем случае один запрос превращается примерно в 202 SQL-запроса: count, rows и до 200 запросов форматов.

Сделать одну batch-выборку форматов по `albumKey IN (...)`, аналогично уже существующей загрузке `albumGenres` в том же методе. Добавить тест с несколькими альбомами и разными форматами и измерить запрос до/после исправления.

### 7. Уточнить масштабные ограничения вместо тихого обрезания

**Подтверждено.** `Catalog.trackIds()` всегда содержит `LIMIT 100000` (`src/server/database.ts:501-509`), а selection schema допускает максимум 100 000 ID (`src/shared/contracts.ts:135-139`). Для проверяемой коллекции ровно в 100 000 треков это незаметно, но более крупная очередь будет молча усечена.

Нужно выбрать и явно оформить контракт:

- либо поддержать пагинацию/потоковую сборку ID для очереди;
- либо оставить лимит безопасности, но возвращать `truncated: true`, показывать предупреждение и не выдавать усечённую выборку за полную.

Операции над selection по фильтру уже могут обрабатываться иначе; не следует без необходимости материализовать сотни тысяч ID в клиенте.

### 8. Измерить и оптимизировать тяжёлые facet-запросы

Часть риска подтверждена структурой кода, но реальный приоритет требует исправленного benchmark:

- `artists()` получает все группы, сортирует их в Node.js и только затем применяет `slice` (`src/server/database.ts:715-737`);
- `facetRelevance()` загружает пути всех подходящих треков и строит каждого предка папки в JavaScript (`src/server/database.ts:539-612`);
- `filterValidity()` последовательно вычисляет полный список жанров, до 100 000 исполнителей и валидные альбомы (`src/server/database.ts:522-537`).

Первый безопасный шаг — перенести сортировку и пагинацию artists в SQLite через уже зарегистрированный `artist_sort_key`. Для папок возможны два варианта после профилирования: кэширование результата по filter key или отдельная нормализованная relation трек–папка, обновляемая при upsert. Изменение схемы оправдано только при измеряемом выигрыше на реальной коллекции.

### 9. Добавить политику обслуживания SQLite

**Подтверждено.** Очереди явно ограничиваются двадцатью записями (`src/server/app.ts:676-681`), но jobs, operations и истёкшие записи `http_cache` только добавляются. UI показывает последние 30 jobs и 100 operations, однако старые строки остаются в базе (`src/server/database.ts:863-898`, `src/server/musicbrainz.ts:519-535`).

Предлагается:

- удалять старые завершённые jobs после безопасного лимита;
- удалять истёкший HTTP-кэш периодически или при старте;
- для operations и recovery ввести одну согласованную пользовательскую команду очистки;
- не удалять данные восстановления автоматически, пока это не объяснено пользователю и не покрыто тестами.

### 10. Сделать CI явным quality gate

Сейчас Windows CI запускает fixtures, tag verification, unit/integration tests и desktop pack (`.github/workflows/ci.yml:20-26`). Typecheck выполняется косвенно внутри `desktop:pack`, но отсутствуют:

- `format:check` без перезаписи файлов;
- ESLint с TypeScript и React Hooks правилами;
- typecheck тестов и JS-скриптов;
- детерминированный браузерный smoke в Chrome/Edge.

Не нужно сразу добавлять весь долгий Playwright-набор в каждый commit. После изоляции fixtures можно выделить короткий smoke для PR, а полный Chrome/Edge-набор запускать на main или перед релизом. Команда `format` должна остаться исправляющей, рядом нужна отдельная `format:check`.
...
4. Отдельным небольшим этапом исправить error semantics, N+1 и явное поведение лимита 100k.
5. Затем декомпозировать App и server по одной feature за коммит, сохраняя внешний API и существующие тесты.
